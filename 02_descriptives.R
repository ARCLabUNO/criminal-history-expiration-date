...
for (i in seq_along(INPUT_FILES)) {
  ...
  if (!is.na(event_date_var)) {
    ...
    # NOTE: already grouped by ResearchID below, so .SD excludes that column;
    # EventDate alone is sufficient to compute the per-person sequence id.
    example_dt[, Sequence := rleidv(.SD, cols = "EventDate"), by = ResearchID]
